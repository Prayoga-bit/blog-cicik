...
class UserGalleryEditor extends Component
{
    public array $items = [];
    ...
    {
        $this->items = $galleryAdminService->getUserEditableItems(auth()->id())
            ->map(function ($item): array {
                return [
                    ...
                    'author_name' => $item->user?->name ?? 'You',
                ];
            })
            ->all();
    }
    ...
    public function saveItem(int $itemId, GalleryAdminService $galleryAdminService): void
    {
        ...
        $gallery = \App\Models\Gallery::findOrFail($itemId);
